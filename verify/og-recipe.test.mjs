...
test("references that leave the repository are not sources", () => {
  const { root } = tree({
    "index.html": `<a href="https://companygraph.io/talks/"></a>
...
// Every page here points at the one copy of fonts/ relatively, so a deck two levels down
// reaches back out of its own directory to reach them. blust.ch's decks carry their own
// fonts/ and never do this; get the resolution wrong and every deck card silently stops
// tracking the fonts it renders with.
...
test("changing the crop changes the recipe", () => {
  const { root } = tree({ "index.html": "<p>one</p>" });
  assert.notEqual(recipe(card({ clipY: 23 }), root), recipe(card(), root));
});
...
// hash is how a new one gets forgotten and starts changing cards silently.
...
// A fourth card added without a card entry would never be checked, and nothing else would say
// so: the check would keep printing three ✓ while the new one drifted.
