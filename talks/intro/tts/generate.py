...
# The narrating voices — one per language, not one per repository. Same ids as the
# sibling repository's talks, and that repetition is deliberate: with a second product
# now narrated, the same two voices become a house voice across the projects rather than
# a signature of either one. What tells the products apart is what the decks say, not who
# reads them — the decks share no vocabulary at all.
#
...
def slides(deck_html):
    ...
    for block in re.split(r'<section class="slide', deck_html)[1:]:
        ...
        # a dry run that wants to write all of them means this mapping is inverted.
