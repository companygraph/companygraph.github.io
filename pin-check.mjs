...
//
// `source.json` names one commit of the meta-model, and that pin is editorial: it says which
// state of the model these pages publish, so moving it is a decision someone makes rather than
// one a tool makes for them. This reports the distance and nothing else — it writes no pin, and
// it never fails the build, because a pin behind its upstream is not a broken site.
//
