...
- [ ] **Step 3a: What `main` brought in.** The site now forbids new tabs everywhere (`noNewTab` parses `data-de` too) and gives the footer credit a lockup. In `example/index.html`: remove `target="_blank" rel="noopener"` (and the single-quoted German form inside any `data-de`) from the source link, the *view file* link in the script, and anywhere else; copy billing's current footer row verbatim — including the `.credit` lockup markup and its CSS (`footer .credit`, `.rbmark`, `.plate`) — so the two pages' footers are byte-identical; in `verify/check.mjs` add `noNewTab: true` to the `/example/` spec. Confirm `noNewTab` passes on `/example/` in both languages.
